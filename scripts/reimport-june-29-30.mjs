...
import { parseCsvTimestamp } from './date-format-utils.js';
...
function parseTimestamp(value) {
  return parseCsvTimestamp(value);
}
...
async function main() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    throw new Error('Missing Supabase config. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in papikopi-web/.env.local.');
  }
  ...
  const start = '2026-06-29T00:00:00+07:00';
  const end = '2026-07-01T00:00:00+07:00';
  ...
  if (saleIds.length > 0) {
    ...
    console.log(`Deleted ${saleIds.length} existing June 29-30 sales rows.`);
  } else {
    console.log('No existing June 29-30 sales rows found to delete.');
  }
  ...
  const baristaId = baristaRows?.[0]?.id ?? null;
  if (!baristaId) throw new Error('No barista user found in the database.');
  ...
  for (const row of rows) {
    ...
    if (day !== '2026-06-29' && day !== '2026-06-30') {
      continue;
    }
    ...
    if (!outletMatch || !productMatch) {
      console.warn(`Skipping row because match failed: outlet=${Boolean(outletMatch)} product=${Boolean(productMatch)} => ${row.cl_role || '-'} / ${row.minuman_id || '-'}`);
      skipped += 1;
      continue;
    }
    ...
    if (saleError) {
      console.error('Failed to insert sale:', saleError.message || saleError);
      skipped += 1;
      continue;
    }
    ...
    const { error: itemError } = await supabase.from('sale_items').insert([itemPayload]);
    if (itemError) {
      console.error('Failed to insert sale item:', itemError.message || itemError);
      skipped += 1;
      continue;
    }
  ...
  console.log(`Imported ${imported} sales and sale_items rows; skipped ${skipped}.`);
}

main().catch((err) => {
  console.error('Reimport script failed:', err);
  process.exit(1);
});
